 ...
 *
 * Description: After the checks, If the file is not
 * an ELF file - exit code 98.
 */
...
/**
 * print_magic - This will prints the magic numbers of
 * an ELF header.
 * @e_ident: A pointer to an array containing the ELF magic numbers.
 *
 ...
 * @argc: This becomes the number of arguments supplied to the program.
 * @argv: An array of pointers to the arguments.
 *
 * Return: This will return  0 on success.
 *
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	dprintf(STDERR_FILENO, "Error: Can't read file %s\n", argv[1]);
	exit(98); }
	r = read(filed, header, sizeof(Elf64_Ehdr));
	if (r == -1)
	{
	free(header);
	close(filed);
